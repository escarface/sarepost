...
func (s Server) handleScheduleHTML(w http.ResponseWriter, r *http.Request) {
	...
	if view != "calendar" && view != "publications" && view != "drafts" && view != "create" && view != "failed" && view != "settings" && view != "generate" && view != "campaigns" && view != "backlog" && view != "sources" {
		view = "calendar"
	}
	...
	contentSourceIncludeArchived := strings.EqualFold(strings.TrimSpace(r.URL.Query().Get("include_archived")), "true")
	contentSources, err := s.Store.ListContentSources(r.Context(), domain.ContentSourceListFilter{
		IncludeArchived: contentSourceIncludeArchived,
		Limit:           maxMCPListLimit,
	})
	if err != nil {
		writeError(w, http.StatusInternalServerError, err)
	...
	if activeNavView == "create" {
		...
		if returnTo != "" {
			if parsed, err := url.Parse(returnTo); err == nil {
				...
				switch sourceView {
				case "publications", "calendar", "drafts", "failed", "settings", "campaigns", "backlog", "sources":
	...
	_ = t.Execute(w, pageData{
		...
		ContentSourceIncludeArchived: contentSourceIncludeArchived,
		...
		ContentSourceCount:           len(contentSources),
